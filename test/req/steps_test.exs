defmodule Req.StepsTest do
  ...
  test "put_params" do
    ...
    assert Req.get!(req, url: "#{url}?x=1&x=2&y=1", params: [x: 9]).body == "x=9&y=1"
    assert Req.get!(req, url: "#{url}?x=1&x=2&y=1", params: [x: 3, x: 4]).body == "x=3&x=4&y=1"
    assert Req.get!(req, url: "#{url}?y=1&x=1", params: [x: 9, z: 3]).body == "y=1&x=9&z=3"
  end
